...
/**
 * Helper: safely parse ingredients from JSON string.
 ...
 *  - name: string
 *  - instructions: string
 *  - alcoholic: string ("true"/"false")
 *  - ingredients: JSON string (array) or omitted
 *  - image: optional file uploaded by multer at req.file
 *
 * Security considerations:
 * - Ingredients are parsed inside a try/catch to prevent crashes
 * - If DB save fails, uploaded file is rolled back (deleted)
...
export const createCocktail = async (req, res) => {
    try {
        ...
        // Build public URL for image if provided
        const serverURL =
        ...
        if (err.code === 11000 && err.keyPattern?.name) {
            return res
                .status(409)
                .json({ error: "This cocktail name is already taken." });
        }
 ...
 * GET /api/cocktails
 * -----------------------------------
 * Returns all cocktails with computed rating stats.
 */
 ...
 * -----------------------------------
 * Returns a single cocktail by id with ratings and stats.
 */
