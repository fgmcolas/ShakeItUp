...
// Ensure the authenticated user matches :id
...
// Sanitize a string array: trim, max length, dedupe
function sanitizeStringArray(arr, maxLen = 64) {
    ...
    for (const s of arr) {
        ...
        const val = s.trim();
        if (!val) continue;
        if (val.length > maxLen) continue; // 64 chars
        if (!seen.has(val)) {
            seen.add(val);
            out.push(val);
        }
...
export const getUserById = async (req, res) => {
    try {
        ...
        if (doPopulate) query = query.populate("favorites");
...
export const toggleFavorite = async (req, res) => {
    try {
        ...
        if (!finalAction) {
            const alreadyFavorite = user.favorites.some((f) => f.toString() === cocktailId);
            finalAction = alreadyFavorite ? "remove" : "add";
        }

        const update =
            finalAction === "add" ? { $addToSet: { favorites: oid } } : { $pull: { favorites: oid } };
        ...
        return res.status(200).json({ favorites: updated.favorites.map((f) => String(f)) });
    } catch (err) {
...
export const updateIngredients = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const sanitized = sanitizeStringArray(req.body?.ingredients, 64); // 64 chars max
