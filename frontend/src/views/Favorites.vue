<script setup>
// Favorites.vue
// Page showing the user's favorite cocktails
// Responsibilities:
// - Fetch full cocktail documents via ?populate=favorites
// - Require a valid user + JWT token
// - Display cocktails in a grid with CocktailCard
...
onMounted(async () => {
    const uid = auth.user.value?._id || auth.user.value?.id // handle both _id and id
    ...
    try {
        ...
        // Ensure favorites is an array of cocktail objects
